...
class Chat(CohereObject):
    ...
    def from_dict(cls, response: Dict[str, Any]) -> "Chat":
        return cls(
            ...
            preamble=response.get("preamble"),
            ...
            tool_calls=ToolCall.from_list(response.get("tool_calls")),
        )
...
class StreamingChat(CohereObject):
    ...
    def _make_response_item(self, index, streaming_item) -> Any:
        ...
        if event_type == StreamEvent.STREAM_START:
            ...
            self.is_search_required = response.get("is_search_required")
            self.citations = response.get("citations")
            self.documents = response.get("documents")
            self.search_results = response.get("search_results")
            self.search_queries = response.get("search_queries")
            self.tool_calls = ToolCall.from_list(response.get("tool_calls"))
